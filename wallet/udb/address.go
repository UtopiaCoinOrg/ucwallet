...
type ManagedAddress interface {
	...
	// Address returns a ucutil.Address for the backing address.
...
type managedAddress struct {
	manager    *Manager
	account    uint32
	address    *ucutil.AddressPubKeyHash
	imported   bool
	internal   bool
	multisig   bool
	compressed bool
...
// Address returns the ucutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() ucutil.Address {
	return a.address
}
...
func newManagedAddressWithoutPrivKey(m *Manager, account uint32, pubKey chainec.PublicKey, compressed bool) (*managedAddress, error) {
	...
	if compressed {
		...
		pubKeyHash = ucutil.Hash160(pubKey.SerializeUncompressed())
	}
	address, err := ucutil.NewAddressPubKeyHash(pubKeyHash, m.chainParams,
...
type scriptAddress struct {
	manager *Manager
	account uint32
	address *ucutil.AddressScriptHash
}
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() ucutil.Address {
	return a.address
}
...
func newScriptAddress(m *Manager, account uint32, scriptHash []byte) (*scriptAddress, error) {
	address, err := ucutil.NewAddressScriptHashFromHash(scriptHash,
		m.chainParams)
	if err != nil {
		return nil, err
	}
