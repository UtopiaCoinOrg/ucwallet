...
type Wallet struct {
	...
	votingEnabled      bool
	balanceToMaintain  ucutil.Amount
	poolAddress        ucutil.Address
	poolFees           float64
	...
	ticketFeeIncrement      ucutil.Amount
	DisallowFree            bool
	AllowHighFees           bool
...
func (w *Wallet) BalanceToMaintain() ucutil.Amount {
...
func (w *Wallet) SetBalanceToMaintain(balance ucutil.Amount) {
	w.stakeSettingsLock.Lock()
...
func (w *Wallet) RelayFee() ucutil.Amount {
	w.relayFeeMu.Lock()
	relayFee := w.relayFee
	w.relayFeeMu.Unlock()
	return relayFee
...
// transaction) used when constructing transactions.
func (w *Wallet) SetRelayFee(relayFee ucutil.Amount) {
	w.relayFeeMu.Lock()
	w.relayFee = relayFee
	w.relayFeeMu.Unlock()
}
...
func (w *Wallet) TicketFeeIncrement() ucutil.Amount {
...
func (w *Wallet) SetTicketFeeIncrement(fee ucutil.Amount) {
	w.ticketFeeIncrementLock.Lock()
...
func (w *Wallet) loadActiveAddrs(ctx context.Context, dbtx walletdb.ReadTx, nb NetworkBackend) (uint64, error) {
	...
	loadBranchAddrs := func(branchKey *hdkeychain.ExtendedKey, n uint32, errs chan<- error) {
		...
		for child := uint32(0); child <= n; child += step {
			child := child
			g.Go(func() error {
				addrs := make([]ucutil.Address, 0, step)
...
func (w *Wallet) CommittedTickets(tickets []*chainhash.Hash) ([]*chainhash.Hash, []ucutil.Address, error) {
	const op errors.Op = "wallet.CommittedTickets"
	hashes := make([]*chainhash.Hash, 0, len(tickets))
	addresses := make([]ucutil.Address, 0, len(tickets))
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for _, v := range tickets {
			...
			var bestAddr ucutil.Address
			var bestAmount ucutil.Amount
...
// message or ucd JSON-RPC request, for the blocks in sidechain and saved in
...
func (w *Wallet) Consolidate(inputs int, account uint32, address ucutil.Address) (*chainhash.Hash, error) {
...
func (w *Wallet) CreateMultisigTx(account uint32, amount ucutil.Amount, pubkeys []*ucutil.AddressSecpPubKey, nrequired int8, minconf int32) (*CreatedTx, ucutil.Address, []byte, error) {
	heldUnlock, err := w.holdUnlock()
	if err != nil {
...
func (w *Wallet) PurchaseTickets(minBalance, spendLimit ucutil.Amount, minConf int32, votingAddr ucutil.Address, account uint32, count int, poolAddress ucutil.Address,
	poolFees float64, expiry int32, txFee ucutil.Amount, ticketFee ucutil.Amount) ([]*chainhash.Hash, error) {
...
type PurchaseTicketsRequest struct {
	...
	minBalance  ucutil.Amount
	spendLimit  ucutil.Amount
	poolAddress ucutil.Address
	poolFees    float64
	txFee       ucutil.Amount
	ticketFee   ucutil.Amount
}
...
// spending to it in the blockchain or ucd mempool), the next chained address
// is returned.
func (w *Wallet) CurrentAddress(account uint32) (ucutil.Address, error) {
...
func (w *Wallet) PubKeyForAddress(a ucutil.Address) (chainec.PublicKey, error) {
...
func (w *Wallet) SignMessage(msg string, addr ucutil.Address) (sig []byte, err error) {
...
func VerifyMessage(msg string, addr ucutil.Address, sig []byte) (bool, error) {
	...
	recoveredAddr, err := ucutil.NewAddressSecpPubKey(serializedPK, ucutil.ActiveNet)
	if err != nil {
		return false, errors.E(op, err)
	}
...
func (w *Wallet) HaveAddress(a ucutil.Address) (bool, error) {
...
func (w *Wallet) AccountOfAddress(a ucutil.Address) (uint32, error) {
...
func (w *Wallet) AddressInfo(a ucutil.Address) (udb.ManagedAddress, error) {
...
func (w *Wallet) MasterPubKey(account uint32) (*hdkeychain.ExtendedKey, error) {
	...
	extKey, err := hdkeychain.NewKeyFromString(masterPubKey, ucutil.ActiveNet)
	if err != nil {
		return nil, errors.E(op, err)
	}
	return extKey, nil
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := ucutil.Amount(output.Value).ToCoin()
		...
		loopDetails:
			for i := range details {
				...
				for _, cred := range detail.Credits {
					...
					apkh, ok := addrs[0].(*ucutil.AddressPubKeyHash)
					if !ok {
						continue
					}
...
func (w *Wallet) Accounts() (*AccountsResult, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*ucutil.Amount)
...
func (w *Wallet) DumpWIFPrivateKey(addr ucutil.Address) (string, error) {
	...
	wif := ucutil.NewWIF(privKey, w.chainParams.PrivateKeyID, ucec.SignatureType(privKey.GetType()))
	return wif.String(), nil
}

// ImportPrivateKey imports a private key to the wallet and writes the new
// wallet to disk.
func (w *Wallet) ImportPrivateKey(wif *ucutil.WIF) (string, error) {
	...
	if n, err := w.NetworkBackend(); err == nil {
		err := n.LoadTxFilter(context.TODO(), false, []ucutil.Address{addr}, nil)
		if err != nil {
			return "", errors.E(op, err)
		}
	}
...
// a P2SH address.
func (w *Wallet) RedeemScriptCopy(addr ucutil.Address) ([]byte, error) {
		...
		for it.Next() {
			...
			if it.SpenderHash != (chainhash.Hash{}) {
				...
				case isVote(spender):
					...
					// earned by fees.
					res.TotalSubsidy += ucutil.Amount(spender.TxIn[0].ValueIn)
...
func (w *Wallet) StakeInfoPrecise(chainClient *rpcclient.Client) (*StakeInfoData, error) {
	...
	mempoolTicketsFuture := chainClient.GetRawMempoolAsync(ucdtypes.GRMTickets)
...
func (w *Wallet) LockedOutpoints() []ucdtypes.TransactionInput {
	w.lockedOutpointMu.Lock()
	locked := make([]ucdtypes.TransactionInput, len(w.lockedOutpoints))
	i := 0
	for op := range w.lockedOutpoints {
		locked[i] = ucdtypes.TransactionInput{
			Txid: op.Hash.String(),
			Vout: op.Index,
		}
		i++
...
func (w *Wallet) SortedActivePaymentAddresses() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr ucutil.Address) error {
...
func ticketMatured(params *chaincfg.Params, txHeight, curHeight int32) bool {
	// ucd has an off-by-one in the calculation of the ticket
...
// returning the total amount of utopia received for a single wallet
// address.
func (w *Wallet) TotalReceivedForAddr(addr ucutil.Address, minConf int32) (ucutil.Amount, error) {
	const op errors.Op = "wallet.TotalReceivedForAddr"
	var amount ucutil.Amount
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType, additionalPrevScripts map[wire.OutPoint][]byte,
	additionalKeysByAddress map[string]*ucutil.WIF, p2shRedeemScriptsByAddress map[string][]byte) ([]SignatureError, error) {
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr ucutil.Address) (
			...
			getScript := txscript.ScriptClosure(func(
				addr ucutil.Address) ([]byte, error) {
...
// address.
func (w *Wallet) CreateSignature(tx *wire.MsgTx, idx uint32, addr ucutil.Address, hashType txscript.SigHashType, prevPkScript []byte) (sig, pubkey []byte, err error) {
...
func (w *Wallet) PublishTransaction(tx *wire.MsgTx, serializedTx []byte, n NetworkBackend) (*chainhash.Hash, error) {
	...
	if err != nil {
		...
		innerErr := err
		for {
			if e, ok := innerErr.(*ucjson.RPCError); ok && e.Code == ucjson.ErrRPCDuplicateTx {
...
func decodeStakePoolColdExtKey(encStr string, params *chaincfg.Params) (map[string]struct{}, error) {
	...
	key, err := hdkeychain.NewKeyFromString(splStrs[0], ucutil.ActiveNet)
	if err != nil {
		return nil, err
	}
...
func Open(cfg *Config) (*Wallet, error) {
	...
	// Amounts
	w.ticketFeeIncrement, err = ucutil.NewAmount(cfg.TicketFee)
	if err != nil {
		return nil, errors.E(op, err)
	}
	w.relayFee, err = ucutil.NewAmount(cfg.RelayFee)
	if err != nil {
		return nil, errors.E(op, err)
	}
