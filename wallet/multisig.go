...
// from them.
func (w *Wallet) MakeSecp256k1MultiSigScript(secp256k1Addrs []ucutil.Address, nRequired int) ([]byte, error) {
	const op errors.Op = "wallet.MakeSecp256k1MultiSigScript"

	secp256k1PubKeys := make([]*ucutil.AddressSecpPubKey, len(secp256k1Addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*ucutil.AddressScriptHash, error) {
	const op errors.Op = "wallet.ImportP2SHRedeemScript"

	var p2shAddr *ucutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if errors.Is(errors.Exist, err) {
				...
				p2shAddr, _ = ucutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*ucutil.AddressScriptHash)
		return nil
	})
	if err != nil {
...
func (w *Wallet) FetchP2SHMultiSigOutput(outPoint *wire.OutPoint) (*P2SHMultiSigOutput, error) {
	...
	p2shAddr, err := ucutil.NewAddressScriptHashFromHash(
