...
// account.
func (w *Wallet) NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb ucutil.Amount, account uint32, minConf int32,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		case OutputSelectionAlgorithmAll:
			...
			inputSource = func(ucutil.Amount) (*txauthor.InputDetail, error) {
				inputDetail, err := sourceImpl.SelectInputs(ucutil.MaxAmount)
...
func (s *secretSource) GetKey(addr ucutil.Address) (chainec.PrivateKey, bool, error) {
...
func (s *secretSource) GetScript(addr ucutil.Address) ([]byte, error) {
...
//
// Utopia: This func also sends the transaction, and if successful, inserts it
...
func (w *Wallet) txToOutputsInternal(op errors.Op, outputs []*wire.TxOut, account uint32, minconf int32,
	n NetworkBackend, randomizeChangeIdx bool, txFee ucutil.Amount) (*txauthor.AuthoredTx, error) {
	...
	if atx.ChangeIndex >= 0 && account == udb.ImportedAddrAccount {
		changeAmount := ucutil.Amount(atx.Tx.TxOut[atx.ChangeIndex].Value)
...
func (w *Wallet) txToMultisig(op errors.Op, account uint32, amount ucutil.Amount, pubkeys []*ucutil.AddressSecpPubKey,
	nRequired int8, minconf int32) (*CreatedTx, ucutil.Address, []byte, error) {
...
func (w *Wallet) txToMultisigInternal(op errors.Op, dbtx walletdb.ReadWriteTx, account uint32, amount ucutil.Amount,
	pubkeys []*ucutil.AddressSecpPubKey, nRequired int8, minconf int32) (*CreatedTx, ucutil.Address, []byte, error) {

	addrmgrNs := dbtx.ReadWriteBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadWriteBucket(wtxmgrNamespaceKey)

	txToMultisigError := func(err error) (*CreatedTx, ucutil.Address, []byte, error) {
	...
	scAddr, err := ucutil.NewAddressScriptHash(msScript, w.chainParams)
	if err != nil {
		return txToMultisigError(errors.E(op, err))
	}
	...
	// Request updates from ucd for new transactions sent to this
	// script hash address.
	err = n.LoadTxFilter(context.TODO(), false, []ucutil.Address{scAddr}, nil)
	if err != nil {
		return txToMultisigError(errors.E(op, err))
	}
...
func (w *Wallet) compressWallet(op errors.Op, maxNumIns int, account uint32, changeAddr ucutil.Address) (*chainhash.Hash, error) {
...
func (w *Wallet) compressWalletInternal(op errors.Op, dbtx walletdb.ReadWriteTx, maxNumIns int, account uint32,
	changeAddr ucutil.Address) (*chainhash.Hash, error) {
...
// passed.
func makeTicket(params *chaincfg.Params, inputPool *extendedOutPoint, input *extendedOutPoint, addrVote ucutil.Address,
	addrSubsidy ucutil.Address, ticketCost int64, addrPool ucutil.Address) (*wire.MsgTx, error) {
	...
	zeroed := [20]byte{}
	addrZeroed, err := ucutil.NewAddressPubKeyHash(zeroed[:], params, 0)
	if err != nil {
		return nil, err
	}
	...
	if addrPool != nil {
		pkScript, vers, err := rewardCommitment(addrPool,
			ucutil.Amount(amountsCommitted[0]), limits)
		if err != nil {
	...
	pkScript, vers, err = rewardCommitment(addrSubsidy,
		ucutil.Amount(amountsCommitted[userSubsidyNullIdx]), limits)
	if err != nil {
		return nil, errors.E(errors.Invalid,
...
func (w *Wallet) purchaseTickets(ctx context.Context, op errors.Op, n NetworkBackend, req *PurchaseTicketsRequest) ([]*chainhash.Hash, error) {
	...
	if w.addressReuse {
		...
		addrFunc = func(errors.Op, persistReturnedChildFunc, uint32) (ucutil.Address, error) {
			return addr, err
		}
	}
	...
	case *ucutil.AddressPubKeyHash, nil:
		stakeSubmissionPkScriptSize = txsizes.P2PKHPkScriptSize + 1
	default:
		return nil, errors.E(op, errors.Invalid,
	...
	var neededPerTicket, ticketFee ucutil.Amount
	var estSize int
	...
	if req.minBalance > 0 {
		...
		estimatedFundsUsed := neededPerTicket * ucutil.Amount(req.Count)
	...
	for i := 0; i < req.Count; i++ {
		...
		// an address.
		var addrVote, addrSubsidy ucutil.Address
		err := walletdb.Update(w.db, func(dbtx walletdb.ReadWriteTx) error {
		...
		if eopPool != nil {
			eopPoolCredit := udb.Credit{
				...
				Amount:       ucutil.Amount(eopPool.amt),
...
func (w *Wallet) findEligibleOutputsAmount(dbtx walletdb.ReadTx, account uint32, minconf int32,
	amount ucutil.Amount, currentHeight int32) ([]udb.Credit, error) {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadBucket(wtxmgrNamespaceKey)
	var outTotal ucutil.Amount
...
func (w *Wallet) signVoteOrRevocation(addrmgrNs walletdb.ReadBucket, ticketPurchase, tx *wire.MsgTx, isVote bool) error {
	...
	// can be performed.
	var getKey txscript.KeyClosure = func(addr ucutil.Address) (chainec.PrivateKey, bool, error) {
		...
		return key, true, nil // secp256k1 pubkeys are always compressed in Utopia
	}
	var getScript txscript.ScriptClosure = func(addr ucutil.Address) ([]byte, error) {
	...
	signedScript, err := txscript.SignTxOutput(w.chainParams, tx, inputToSign,
		...
		tx.TxIn[inputToSign].SignatureScript, ucec.STEcdsaSecp256k1)
	if err != nil {
...
func createUnsignedRevocation(ticketHash *chainhash.Hash, ticketPurchase *wire.MsgTx, feePerKB ucutil.Amount) (*wire.MsgTx, error) {
	// Parse the ticket purchase transaction to determine the required output
	// destinations for vote rewards or revocations.
	ticketPayKinds, ticketHash160s, ticketValues, _, _, _ :=
		stake.TxSStxStakeOutputInfo(ticketPurchase)
	...
	for _, output := range revocation.TxOut {
		if ucutil.Amount(output.Value) > feeEstimate {
			amount := ucutil.Amount(output.Value) - feeEstimate
