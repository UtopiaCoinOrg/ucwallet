...
// method.  This may be a request that is handled directly by ucwallet, or
// a chain server request that is handled by passing the request down to ucd.
//
...
func (s *Server) handlerClosure(ctx context.Context, request *ucjson.Request) lazyHandler {
...
// but this is only implemented here to work around an oddity with ucjson,
...
func (s *Server) invalidAuth(req *ucjson.Request) bool {
	cmd, err := ucjson.ParseParams(types.Method(req.Method), req.Params)
	if err != nil {
		return false
	}
	authCmd, ok := cmd.(*ucdtypes.AuthenticateCmd)
	if !ok {
		return false
	}
	// Check credentials.
	...
	for {
		...
		case reqBytes, ok := <-wsc.allRequests:
			...
			default:
				...
				go func() {
					resp, jsonErr := f()
					mresp, err := ucjson.MarshalResponse(req.Jsonrpc, req.ID, resp, jsonErr)
					if err != nil {
...
func (s *Server) postClientRPC(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		resp, err := ucjson.MarshalResponse(req.Jsonrpc, req.ID, nil, ucjson.ErrRPCInvalidRequest)
		if err != nil {
			log.Errorf("Unable to marshal response to client %s: %v",
				r.RemoteAddr, err)
	...
	case "stop":
		...
		stop = true
		res = "ucwallet stopping"
	default:
		res, jsonErr = s.handlerClosure(ctx, &req)()
	}

	// Marshal and send.
	mresp, err := ucjson.MarshalResponse(req.Jsonrpc, req.ID, res, jsonErr)
	if err != nil {
		log.Errorf("Unable to marshal response to client %s: %v",
			r.RemoteAddr, err)
