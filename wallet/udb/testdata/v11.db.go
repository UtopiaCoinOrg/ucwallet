...
func pay2ssgen(addr ucutil.Address) []byte {
...
func pay2ssrtx(addr ucutil.Address) []byte {
...
func pay2sstx(addr ucutil.Address) []byte {
...
func sstxAddrPush(addr ucutil.Address, amount int64) []byte {
	s, err := txscript.GenerateSStxAddrPush(addr, ucutil.Amount(amount), 0x0058)
	if err != nil {
		panic(err)
	}
	return s
...
func setup() error {
	...
	return walletdb.Update(db, func(tx walletdb.ReadWriteTx) error {
		...
		poolFeeAddr, _ := ucutil.DecodeAddress("TsR28UZRprhgQQhzWns2M6cAwchrNVvbYq2")
		txinIdx := uint32(0)
		...
		nextAddress := func() (ucutil.Address, error) {
			lastAcctIdx++
