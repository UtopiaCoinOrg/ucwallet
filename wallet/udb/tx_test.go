...
func TestInsertsCreditsDebitsRollbacks(t *testing.T) {
	...
	b1H := g.generate(ucutil.BlockValid)
	b1Hash := b1H.BlockHash()
	b2H := g.generate(ucutil.BlockValid)
	b2Hash := b2H.BlockHash()
	b3H := g.generate(ucutil.BlockValid)
	...
	tests := []struct {
		...
		},
		{
			...
			bal: 0,
			unc: ucutil.Amount(tx1Rec.MsgTx.TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  tx1Rec.Hash,
					Index: 0,
		...
		},
		{
			...
			bal: ucutil.Amount(tx1Rec.MsgTx.TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  tx1Rec.Hash,
...
func TestCoinbases(t *testing.T) {
	...
	for idx := 0; idx < 18; idx++ {
		bh := g.generate(ucutil.BlockValid)
	...
	err = walletdb.Update(db, func(tx walletdb.ReadWriteTx) error {
		...
		expectedImmature := []coinbaseTest{
			{
				immature:  ucutil.Amount(50e8),
				spendable: ucutil.Amount(0),
			},
		}
		...
		expectedMature := []coinbaseTest{
			{
				immature:  ucutil.Amount(0),
				spendable: ucutil.Amount(50e8),
			},
		}
