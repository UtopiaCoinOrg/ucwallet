...
// notifications from a ucd JSON-RPC server.
...
// the ucd server are performed.  Otherwise, it will listen for notifications
...
func (s *RPCSyncer) Run(ctx context.Context, startupSync bool) error {
	...
	g.Go(func() error {
		...
		if err != nil {
			const op errors.Op = "ucd.jsonrpc.notifyspentandmissedtickets"
			return errors.E(op, err)
		}
	...
	for {
		...
		case n, ok := <-c:
			...
			case relevantTxAccepted:
				op = "ucd.jsonrpc.relevanttxaccepted"
				nonFatal = true
				var rpt *chainhash.Hash
				rpt, err = s.wallet.RescanPoint()
				if err != nil || rpt != nil {
...
func (s *RPCSyncer) startupSync(ctx context.Context) error {
	...
	if err != nil {
		const op errors.Op = "ucd.jsonrpc.notifyblocks"
		return errors.E(op, err)
	}
	...
	for {
		...
		err := ctxdo(ctx, "ucd.jsonrpc.getheaders", func() error {
	...
	if err != nil {
		const op errors.Op = "ucd.jsonrpc.rebroadcastwinners"
		return errors.E(op, err)
	}
	_, err = s.rpcClient.RawRequest("rebroadcastmissed", nil)
	if err != nil {
		const op errors.Op = "ucd.jsonrpc.rebroadcastmissed"
		return errors.E(op, err)
	}
