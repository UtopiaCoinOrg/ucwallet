...
func deserializeSStxRecord(serializedSStxRecord []byte, dbVersion uint32) (*sstxRecord, error) {
	...
	case dbVersion < 3:
		...
		// Create and save the ucutil.Tx of the read MsgTx and set its index.
		tx := ucutil.NewTx(msgTx)
		tx.SetIndex(ucutil.TxIndexUnknown)
	...
	case dbVersion >= 3:
		...
		return &sstxRecord{tx: ucutil.NewTx(&tx), ts: time.Unix(unixTime, 0)}, nil
