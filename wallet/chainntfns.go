...
func (w *Wallet) evaluateStakePoolTicket(rec *udb.TxRecord, blockHeight int32, poolUser ucutil.Address) bool {
	tx := rec.MsgTx
	...
	out := ucutil.Amount(0)
	for i := range tx.TxOut {
		out += ucutil.Amount(tx.TxOut[i].Value)
	}
	fees := in - out
	...
	if exists {
		...
		feeNeeded := txrules.StakePoolTicketFee(ucutil.Amount(
...
func (w *Wallet) processTransactionRecord(dbtx walletdb.ReadWriteTx, rec *udb.TxRecord,
	...
	if stake.IsSStx(&rec.MsgTx) {
		...
		if insert {
			err := w.StakeMgr.InsertSStx(stakemgrNs, ucutil.NewTx(&rec.MsgTx))
			if err != nil {
...
func (w *Wallet) VoteOnOwnedTickets(winningTicketHashes []*chainhash.Hash, blockHash *chainhash.Hash, blockHeight int32) error {
	...
	if err != nil {
		...
		rpcErr, ok := err.(*ucjson.RPCError)
		if !ok || rpcErr.Code != ucjson.ErrRPCDuplicateTx {
