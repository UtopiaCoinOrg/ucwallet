...
func writeLocaleHelp(locale, goLocale string, descs map[string]string) {
	...
	for i := range rpchelp.Methods {
		m := &rpchelp.Methods[i]
		helpText, err := ucjson.GenerateHelp(types.Method(m.Method), descs, m.ResultTypes...)
		if err != nil {
			log.Fatal(err)
		}
...
func writeUsage() {
	...
	var err error
	for i := range rpchelp.Methods {
		usageStrs[i], err = ucjson.MethodUsageText(types.Method(rpchelp.Methods[i].Method))
		if err != nil {
			log.Fatal(err)
		}
	}
