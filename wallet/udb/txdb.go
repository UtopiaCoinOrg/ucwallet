...
func fetchMinedBalance(ns walletdb.ReadBucket) (ucutil.Amount, error) {
	v := ns.Get(rootMinedBalance)
	if len(v) != 8 {
		return 0, errors.E(errors.IO, errors.Errorf("mined balance len %d", len(v)))
	}
	return ucutil.Amount(byteOrder.Uint64(v)), nil
}

func putMinedBalance(ns walletdb.ReadWriteBucket, amt ucutil.Amount) error {
	v := make([]byte, 8)
...
func fetchRawCreditAmount(v []byte) (ucutil.Amount, error) {
	if len(v) < 9 {
...
// credit is spent.
func fetchRawCreditAmountSpent(v []byte) (ucutil.Amount, bool, error) {
	if len(v) < 9 {
		return 0, false, errors.E(errors.IO, errors.Errorf("credit len %d", len(v)))
	}
	return ucutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<0) != 0, nil
}

// fetchRawCreditAmountChange returns the amount of the credit and whether the
// credit is marked as change.
func fetchRawCreditAmountChange(v []byte) (ucutil.Amount, bool, error) {
	if len(v) < 9 {
		return 0, false, errors.E(errors.IO, errors.Errorf("credit len %d", len(v)))
	}
	return ucutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<1) != 0, nil
}
...
// amount is returned.
func spendCredit(ns walletdb.ReadWriteBucket, k []byte, spender *indexedIncidence) (ucutil.Amount, error) {
	...
	return ucutil.Amount(byteOrder.Uint64(v[0:8])), putRawCredit(ns, k, v)
}
...
func unspendRawCredit(ns walletdb.ReadWriteBucket, k []byte) (ucutil.Amount, error) {
	...
	return ucutil.Amount(byteOrder.Uint64(v[0:8])), nil
}
...
func (it *creditIterator) readElem() error {
	...
	it.elem.Amount = ucutil.Amount(byteOrder.Uint64(it.cv))
...
func valueDebit(amount ucutil.Amount, credKey []byte) []byte {
	v := make([]byte, 80)
...
func putDebit(ns walletdb.ReadWriteBucket, txHash *chainhash.Hash, index uint32, amount ucutil.Amount, block *Block, credKey []byte) error {
...
func extractRawDebitAmount(v []byte) ucutil.Amount {
	return ucutil.Amount(byteOrder.Uint64(v[:8]))
}
...
func valueUnminedCredit(amount ucutil.Amount, change bool, opCode uint8,
...
func fetchRawUnminedCreditAmount(v []byte) (ucutil.Amount, error) {
...
func fetchRawUnminedCreditAmountChange(v []byte) (ucutil.Amount, bool, error) {
	if len(v) < unconfValueSizeLegacy {
		return 0, false, errors.E(errors.IO, errors.Errorf("unmined credit len %d", len(v)))
	}
	amt := ucutil.Amount(byteOrder.Uint64(v))
	change := v[8]&(1<<1) != 0
...
func valueMultisigOut(sh [ripemd160.Size]byte, m uint8, n uint8,
	...
	blockHeight uint32, amount ucutil.Amount, spentBy chainhash.Hash,
...
func fetchMultisigOut(k, v []byte) (*MultisigOut, error) {
	...
	mso.Amount = ucutil.Amount(byteOrder.Uint64(v[59:67]))
...
func fetchMultisigOutAmount(v []byte) ucutil.Amount {
	return ucutil.Amount(byteOrder.Uint64(v[59:67]))
}
...
func valueTicketCommitment(amount ucutil.Amount, account uint32) []byte {
	v := make([]byte, 12)
	byteOrder.PutUint64(v, uint64(amount))
...
func fetchRawTicketCommitmentAmount(v []byte) (ucutil.Amount, error) {
	if len(v) < 8 {
