...
import (
	...
	"github.com/Utopia/ucwallet/internal/helpers"
	"github.com/Utopia/ucwallet/p2p"
	"github.com/Utopia/ucwallet/rpc/jsonrpc/types"
	ver "github.com/Utopia/ucwallet/version"
...
var handlers = map[string]handler{
	...
	// Reference methods which can't be implemented by ucwallet due to
...
// unsupported by ucwallet due to design differences.
func unsupported(*Server, context.Context, interface{}) (interface{}, error) {
	return nil, &ucjson.RPCError{
		Code:    -1,
		Message: "Request unsupported by ucwallet",
	}
}
...
// context.
type lazyHandler func() (interface{}, *ucjson.RPCError)
...
func lazyApplyHandler(s *Server, ctx context.Context, request *ucjson.Request) lazyHandler {
	handlerData, ok := handlers[request.Method]
	if !ok {
		return func() (interface{}, *ucjson.RPCError) {
			...
			if err != nil {
				return nil, rpcErrorf(ucjson.ErrRPCClientNotConnected, "RPC passthrough requires ucd RPC synchronization")
			}
			var resp interface{}
	...
	return func() (interface{}, *ucjson.RPCError) {
		...
		if err != nil {
			return nil, ucjson.ErrRPCInvalidRequest
		}
...
func makeResponse(id, result interface{}, err error) ucjson.Response {
	idPtr := idPointer(id)
	if err != nil {
		return ucjson.Response{
			ID:    idPtr,
...
func (s *Server) accountAddressIndex(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	case udb.InternalBranch:
		return intChild, nil
	default:
		return nil, rpcErrorf(ucjson.ErrRPCInvalidParameter, "invalid branch %v", cmd.Branch)
	}
}
...
func (s *Server) accountSyncAddressIndex(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	if index >= hdkeychain.HardenedKeyStart {
		return nil, rpcErrorf(ucjson.ErrRPCInvalidParameter,
...
func makeMultiSigScript(w *wallet.Wallet, keys []string, nRequired int) ([]byte, error) {
	keysesPrecious := make([]*ucutil.AddressSecpPubKey, len(keys))
	...
	for i, a := range keys {
		...
		default:
			...
			if ucec.SignatureType(pubKey.GetType()) != ucec.STEcdsaSecp256k1 {
				return nil, rpcErrorf(ucjson.ErrRPCInvalidParameter,
					"only secp256k1 pubkeys are currently supported")
			}
			pubKeyAddr, err := ucutil.NewAddressSecpPubKey(
				pubKey.Serialize(), w.ChainParams())
			if err != nil {
				return nil, rpcError(ucjson.ErrRPCInvalidAddressOrKey, err)
			}
...
func (s *Server) addMultiSigAddress(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	secp256k1Addrs := make([]ucutil.Address, len(cmd.Keys))
	...
	err = n.LoadTxFilter(ctx, false, []ucutil.Address{p2shAddr}, nil)
	if err != nil {
		return nil, err
	}
...
func (s *Server) addTicket(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	if err != nil {
		return nil, rpcError(ucjson.ErrRPCDeserialization, err)
	}
...
func (s *Server) createMultiSig(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	address, err := ucutil.NewAddressScriptHash(script, w.ChainParams())
	if err != nil {
		return nil, err
	}
...
func (s *Server) generateVote(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	if err != nil {
		return nil, rpcError(ucjson.ErrRPCDecodeHexString, err)
	}
...
func (s *Server) getBalance(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	if minConf < 0 {
		return nil, rpcErrorf(ucjson.ErrRPCInvalidParameter, "minconf must be non-negative")
	}
	...
	if accountName == "*" {
		...
		var (
			totImmatureCoinbase ucutil.Amount
			totImmatureStakegen ucutil.Amount
			totLocked           ucutil.Amount
			totSpendable        ucutil.Amount
		...
		for _, bal := range balances {
			...
			if err != nil {
				...
				if errors.Is(errors.NotExist, err) {
					return nil, rpcError(ucjson.ErrRPCInternal.Code, err)
				}
				return nil, err
			}
...
func (s *Server) getBestBlock(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	result := &ucdtypes.GetBestBlockResult{
		Hash:   hash.String(),
		Height: int64(height),
	}
	return result, nil
...
func (s *Server) getBlockHash(ctx context.Context, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*ucdtypes.GetBlockHashCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func (s *Server) getInfo(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	var spendableBalance ucutil.Amount
...
func decodeAddress(s string, params *chaincfg.Params) (ucutil.Address, error) {
	...
	if len(s) == 66 || len(s) == 130 {
		...
		pubKeyAddr, err := ucutil.NewAddressSecpPubKey(pubKeyBytes,
			params)
		if err != nil {
			return nil, err
		}
...
// appear in the blockchain, or any tx that has arrived in the ucd mempool).
// If the most recently-requested address has been used, a new address (the
// next chained address in the keypool) is used.  This can fail if the keypool
// runs out (and will return ucjson.ErrRPCWalletKeypoolRanOut if that happens).
...
func (s *Server) importPrivKey(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	wif, err := ucutil.DecodeWIF(cmd.PrivKey, ucutil.ActiveNet.PrivateKeyID)
	if err != nil {
		return nil, rpcErrorf(ucjson.ErrRPCInvalidAddressOrKey, "WIF decode failed: %v", err)
	}
	if !wif.IsForNet(w.ChainParams()) {
		return nil, rpcErrorf(ucjson.ErrRPCInvalidAddressOrKey, "key is not intended for %s", w.ChainParams().Name)
	}
...
func (s *Server) importScript(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	if len(rs) == 0 {
		return nil, rpcErrorf(ucjson.ErrRPCInvalidParameter, "empty script")
	}
...
// keypoolRefill handles the keypoolrefill command.  ucwallet generates
...
func (s *Server) createNewAccount(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	if err != nil {
		if errors.Is(errors.Locked, err) {
			return nil, rpcErrorf(ucjson.ErrRPCWalletUnlockNeeded, "creating new accounts requires an unlocked wallet")
		}
		return nil, err
	}
	return nil, nil
...
func (s *Server) getNewAddress(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	if cmd.GapPolicy != nil {
		...
		default:
			return nil, rpcErrorf(ucjson.ErrRPCInvalidParameter, "unknown gap policy %q", *cmd.GapPolicy)
		}
	}
...
func (s *Server) getTransaction(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	if errors.Is(errors.NotExist, err) {
		return nil, rpcErrorf(ucjson.ErrRPCNoTxInfo, "no information for transaction")
	} else if err != nil {
		return nil, err
	}
	...
	var (
		debitTotal  ucutil.Amount
		creditTotal ucutil.Amount
		fee         ucutil.Amount
		negFeeF64   float64
	)
	...
	if len(txd.Debits) == len(txd.MsgTx.TxIn) {
		var outputTotal ucutil.Amount
		for _, output := range txd.MsgTx.TxOut {
			outputTotal += ucutil.Amount(output.Value)
		}
...
func (s *Server) help(ctx context.Context, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*ucdtypes.HelpCmd)
	// TODO: The "help" RPC should use a HTTP POST client when calling down to
	// ucd for additional help methods.  This avoids including websocket-only
	...
	return nil, rpcErrorf(ucjson.ErrRPCInvalidParameter, "no help for method %q", *cmd.Command)
}
...
func (s *Server) listSinceBlock(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	if targetConf < 1 {
		return nil, rpcErrorf(ucjson.ErrRPCInvalidParameter, "target_confirmations must be positive")
	}
...
func (s *Server) listScripts(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	for i, redeemScript := range redeemScripts {
		p2shAddr, err := ucutil.NewAddressScriptHash(redeemScript,
			w.ChainParams())
		if err != nil {
			return nil, err
		}
...
func (s *Server) purchaseTicket(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	spendLimit, err := ucutil.NewAmount(cmd.SpendLimit)
	if err != nil {
		return nil, rpcError(ucjson.ErrRPCInvalidParameter, err)
	}
	if spendLimit < 0 {
		return nil, rpcErrorf(ucjson.ErrRPCInvalidParameter, "negative spend limit")
	}
	...
	if cmd.MinConf != nil {
		minConf = int32(*cmd.MinConf)
		if minConf < 0 {
			return nil, rpcErrorf(ucjson.ErrRPCInvalidParameter, "negative minconf")
		}
	}

	// Set ticket address if specified.
	var ticketAddr ucutil.Address
	...
	if cmd.PoolAddress != nil {
		if *cmd.PoolAddress != "" {
			...
			// the pool after.
			if cmd.PoolFees == nil {
				return nil, rpcErrorf(ucjson.ErrRPCInvalidParameter, "pool address set without pool fee")
			}
			poolFee = *cmd.PoolFees
			if !txrules.ValidPoolFeeRate(poolFee) {
				return nil, rpcErrorf(ucjson.ErrRPCInvalidParameter, "pool fee percentage %v", poolFee)
			}
		}
	}
	...
	if cmd.TicketFee != nil {
		ticketFee, err = ucutil.NewAmount(*cmd.TicketFee)
		if err != nil {
			return nil, rpcError(ucjson.ErrRPCInvalidParameter, err)
		}
	}
...
func addressScript(addr ucutil.Address) (pkScript []byte, version uint16, err error) {
...
// and amounts.
func makeOutputs(pairs map[string]ucutil.Amount, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
...
func sendPairs(w *wallet.Wallet, amounts map[string]ucutil.Amount, account uint32, minconf int32) (string, error) {
...
func (s *Server) redeemMultiSigOuts(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	p2shAddr, ok := addr.(*ucutil.AddressScriptHash)
	if !ok {
		return nil, rpcErrorf(ucjson.ErrRPCInvalidParameter, "address is not P2SH")
	}
...
func (s *Server) ticketsForAddress(ctx context.Context, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*ucdtypes.TicketsForAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	return ucdtypes.TicketsForAddressResult{Tickets: ticketHashStrs}, nil
}
...
func (s *Server) sendMany(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	// Recreate address/amount pairs, using ucutil.Amount.
	pairs := make(map[string]ucutil.Amount, len(cmd.Amounts))
	for k, v := range cmd.Amounts {
		amt, err := ucutil.NewAmount(v)
		if err != nil {
			return nil, rpcError(ucjson.ErrRPCInvalidParameter, err)
		}
		pairs[k] = amt
...
func (s *Server) sendToMultiSig(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	account := uint32(udb.DefaultAccountNum)
	amount, err := ucutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, rpcError(ucjson.ErrRPCInvalidParameter, err)
	}
	nrequired := int8(*cmd.NRequired)
	minconf := int32(*cmd.MinConf)
	pubkeys := make([]*ucutil.AddressSecpPubKey, len(cmd.Pubkeys))
...
func (s *Server) setTicketFee(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	if cmd.Fee < 0 {
		return nil, rpcErrorf(ucjson.ErrRPCInvalidParameter, "negative fee")
	}

	incr, err := ucutil.NewAmount(cmd.Fee)
	if err != nil {
		return nil, rpcError(ucjson.ErrRPCInvalidParameter, err)
	}
...
func (s *Server) setTxFee(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	relayFee, err := ucutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, rpcError(ucjson.ErrRPCInvalidParameter, err)
	}
...
func (s *Server) signRawTransaction(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	case "ssrtx": // Special case of SigHashAll
		hashType = txscript.SigHashAll
	default:
		return nil, rpcErrorf(ucjson.ErrRPCInvalidParameter, "invalid sighash flag")
	}

	// TODO: really we probably should look these up with ucd anyway to
	...
	for _, rti := range cmdInputs {
		...
		if cmd.PrivKeys != nil && len(*cmd.PrivKeys) != 0 {
			...
			addr, err := ucutil.NewAddressScriptHash(redeemScript,
				w.ChainParams())
			if err != nil {
				return nil, err
			}
	...
	// querying ucd with getrawtransaction. We queue up a bunch of async
	...
	var keys map[string]*ucutil.WIF
	if cmd.PrivKeys != nil {
		keys = make(map[string]*ucutil.WIF)

		for _, key := range *cmd.PrivKeys {
			wif, err := ucutil.DecodeWIF(key, ucutil.ActiveNet.PrivateKeyID)
			if err != nil {
				return nil, rpcError(ucjson.ErrRPCDeserialization, err)
			}

			if !wif.IsForNet(w.ChainParams()) {
				return nil, rpcErrorf(ucjson.ErrRPCInvalidParameter, "key intended for different network")
			}

			var addr ucutil.Address
			switch wif.DSA() {
			case ucec.STEcdsaSecp256k1:
				addr, err = ucutil.NewAddressSecpPubKey(wif.SerializePubKey(),
					w.ChainParams())
				if err != nil {
					return nil, err
				}
			case ucec.STEd25519:
				addr, err = ucutil.NewAddressEdwardsPubKey(
					wif.SerializePubKey(),
					w.ChainParams())
				if err != nil {
					return nil, err
			...
			case ucec.STSchnorrSecp256k1:
				addr, err = ucutil.NewAddressSecSchnorrPubKey(
					wif.SerializePubKey(),
					w.ChainParams())
				if err != nil {
					return nil, err
	...
	for outPoint, resp := range requested {
		...
		if err != nil {
			return nil, errors.E(errors.Op("ucd.jsonrpc.gettxout"), err)
		}
...
func (s *Server) sweepAccount(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	if cmd.FeePerKb != nil {
		var err error
		feePerKb, err = ucutil.NewAmount(*cmd.FeePerKb)
		if err != nil {
			return nil, rpcError(ucjson.ErrRPCInvalidParameter, err)
		}
	}
	...
	if err != nil {
		if errors.Is(errors.InsufficientBalance, err) {
			return nil, rpcError(ucjson.ErrRPCWalletInsufficientFunds, err)
		}
		return nil, err
	}
...
func (s *Server) validateAddress(ctx context.Context, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*ucdtypes.ValidateAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func (s *Server) verifyMessage(ctx context.Context, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*ucdtypes.VerifyMessageCmd)
	...
	case *ucutil.AddressPubKeyHash:
		if a.DSA() != ucec.STEcdsaSecp256k1 {
			goto WrongAddrKind
		}
	default:
		goto WrongAddrKind
...
WrongAddrKind:
	return nil, rpcErrorf(ucjson.ErrRPCInvalidParameter, "address must be secp256k1 P2PK or P2PKH")
}
...
func (s *Server) version(ctx context.Context, icmd interface{}) (interface{}, error) {
	var resp map[string]ucdtypes.VersionResult
	n, _ := s.walletLoader.NetworkBackend()
	chainClient, err := chain.RPCClientFromBackend(n)
	if err == nil {
		var err error
		...
		resp = make(map[string]ucdtypes.VersionResult)
	}

	resp["ucwalletjsonrpcapi"] = ucdtypes.VersionResult{
...
func (s *Server) walletPassphraseChange(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	if err != nil {
		if errors.Is(errors.Passphrase, err) {
			return nil, rpcErrorf(ucjson.ErrRPCWalletPassphraseIncorrect, "incorrect passphrase")
		}
		return nil, err
	}
	return nil, nil
...
func decodeHexStr(hexStr string) ([]byte, error) {
	...
	if err != nil {
		return nil, rpcErrorf(ucjson.ErrRPCDecodeHexString, "hex string decode failed: %v", err)
	}
	return decoded, nil
}
