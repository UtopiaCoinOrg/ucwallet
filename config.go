...
const (
	defaultCAFilename              = "ucd.cert"
	defaultConfigFilename          = "ucwallet.conf"
	defaultLogLevel                = "info"
	defaultLogDirname              = "logs"
	defaultLogFilename             = "ucwallet.log"
...
var (
	ucdDefaultCAFile  = filepath.Join(ucutil.AppDataDir("ucd", false), "rpc.cert")
	defaultAppDataDir  = ucutil.AppDataDir("ucwallet", false)
...
type config struct {
	...
	// RPC client options
	RPCConnect       string                  `short:"c" long:"rpcconnect" description:"Network address of ucd RPC server"`
	CAFile           *cfgutil.ExplicitString `long:"cafile" description:"ucd RPC Certificate Authority"`
	DisableClientTLS bool                    `long:"noclienttls" description:"Disable TLS for ucd RPC; only allowed when connecting to localhost"`
	UcdUsername     string                  `long:"ucdusername" description:"ucd RPC username; overrides --username"`
	UcdPassword     string                  `long:"ucdpassword" default-mask:"-" description:"ucd RPC password; overrides --password"`
	Proxy            string                  `long:"proxy" description:"Perform ucd RPC via SOCKS5 proxy (e.g. 127.0.0.1:9050)"`
	...
	Username               string                  `short:"u" long:"username" description:"JSON-RPC username and default ucd RPC username"`
	Password               string                  `short:"P" long:"password" default-mask:"-" description:"JSON-RPC password and default ucd RPC password"`
...
//
// The above results in ucwallet functioning properly without any config
...
func loadConfig(ctx context.Context) (*config, []string, error) {
	...
	ucutil.ActiveNet = activeNet.Params
	if numNets > 1 {
	...
	if cfg.DisableClientTLS {
		...
		// If CAFile is unset, choose either the copy or local ucd cert.
		if !cfg.CAFile.ExplicitlySet() {
			cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

			// If the CA copy does not exist, check if we're connecting to
			// a local ucd and switch to its RPC cert if it exists.
			...
			if !certExists {
				if _, ok := localhostListeners[RPCHost]; ok {
					ucdCertExists, err := cfgutil.FileExists(
						ucdDefaultCAFile)
					if err != nil {
						fmt.Fprintln(os.Stderr, err)
						return loadConfigError(err)
					...
					if ucdCertExists {
						cfg.CAFile.Value = ucdDefaultCAFile
					}
				}
			}
		}
	...
	// If the ucd username or password are unset, use the same auth as for
	// the client.  The two settings were previously shared for ucd and
	// client auth, so this avoids breaking backwards compatibility while
	// allowing users to use different auth settings for ucd and wallet.
	if cfg.UcdUsername == "" {
		cfg.UcdUsername = cfg.Username
	}
	if cfg.UcdPassword == "" {
		cfg.UcdPassword = cfg.Password
	}
