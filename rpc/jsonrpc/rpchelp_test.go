...
func TestRPCMethodHelpGeneration(t *testing.T) {
	...
	for i := range rpchelp.HelpDescs {
		...
		for _, m := range rpchelp.Methods {
			...
			helpText, err := ucjson.GenerateHelp(m.Method, rpchelp.HelpDescs[i].Descs, m.ResultTypes...)
			if err != nil {
...
func TestRPCMethodUsageGeneration(t *testing.T) {
	...
	for _, m := range rpchelp.Methods {
		...
		usage, err := ucjson.MethodUsageText(m.Method)
		if err != nil {
...
func TestAddress(t *testing.T) {
	...
	/*
	Uk11PPEokc8ubmPiSwVqKFtjHD9e1y2N1TkGWAej57n6i9rvUYrCP
UkzzPPEokc8ubmPiSwVqKFtjHD9e1y2N1TkGWAej57n6i9rvUYrCP
	*/
	addr, _ := decodeAddress("Rs119Uibq94Rao5JE6mG2GhZKT4cCXaeYQV", ucutil.ActiveNet)
	addr2, _ := decodeAddress("Rszz9Uibq94Rao5JE6mG2GhZKT4cCXaeYQV", ucutil.ActiveNet)
