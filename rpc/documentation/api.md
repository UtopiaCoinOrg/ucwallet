...
the management of the wallet and its connection to the Utopia network.  It has
...
- `string network_address`: The host/IP and optional port of the RPC server to
  ...
  default port is chosen corresponding to the default ucd RPC port of the
  active Utopia network.
...
- `string public_key`: The public key encoded as a string in the Utopia encoding
  format.
...
the Utopia network.  If the transaction spends any of the wallet's unspent
