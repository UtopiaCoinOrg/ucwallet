...
(account 0) from a testnet wallet listening on `localhost:11511` in several
different languages:
...
func main() {
	...
	conn, err := grpc.Dial("localhost:11511", grpc.WithTransportCredentials(creds))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	fmt.Println("Spendable balance: ", ucutil.Amount(balanceResponse.Spendable))
}
```
...
```bash
$ protoc -I/path/to/ucwallet/rpc --cpp_out=. --grpc_out=. \
  --plugin=protoc-gen-grpc=$(which grpc_cpp_plugin) \
  /path/to/ucwallet/rpc/api.proto
```
...
auto main() -> int {
    ...
    // configured to use ECDSA TLS certificates (required by ucwallet).
    ...
    auto wallet_tls_cert_file = []{
        ...
        return pw->pw_dir + "/.ucwallet/rpc.cert"s;
    }();
    ...
    auto channel = grpc::CreateChannel("localhost:11511", creds);
...
```PowerShell
PS> & protoc.exe -I \Path\To\ucwallet\rpc --csharp_out=Example --grpc_out=Example `
...
namespace Example
{
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            // certificates (required by ucwallet).
            Environment.SetEnvironmentVariable("GRPC_SSL_CIPHER_SUITES", "HIGH+ECDSA");

            var walletAppData = Portability.LocalAppData(Environment.OSVersion.Platform, "Ucwallet");
            var walletTlsCertFile = Path.Combine(walletAppData, "rpc.cert");
            var cert = await FileUtils.ReadFileAsync(walletTlsCertFile);
            var channel = new Channel("localhost:11511", new SslCredentials(cert));
            try
            {
...
if (os.platform() == 'win32') {
  certPath = path.join(process.env.LOCALAPPDATA, 'Ucwallet', 'rpc.cert');
} else if (os.platform() == 'darwin') {
  certPath = path.join(process.env.HOME, 'Library', 'Application Support',
    'Ucwallet', 'rpc.cert');
} else {
  certPath = path.join(process.env.HOME, '.ucwallet', 'rpc.cert');
}

var cert = fs.readFileSync(certPath);
var creds = grpc.credentials.createSsl(cert);
var client = new walletrpc.WalletService('localhost:11511', creds);
...
```bash
$ protoc -I /path/to/Utopia/ucwallet/rpc --python_out=. --grpc_out=. \
...
def main():
    ...
    if platform.system() == 'Windows':
        cert_file_path = os.path.join(os.environ['LOCALAPPDATA'], "Ucwallet", "rpc.cert")
    elif platform.system() == 'Darwin':
        cert_file_path = os.path.join(os.environ['HOME'], 'Library', 'Application Support',
                                      'Ucwallet', 'rpc.cert')
    else:
        cert_file_path = os.path.join(os.environ['HOME'], '.ucwallet', 'rpc.cert')

    with open(cert_file_path, 'r') as f:
        cert = f.read()
    creds = implementations.ssl_client_credentials(cert, None, None)
    channel = implementations.secure_channel('localhost', 11511, creds)
