...
var (
	walletDataDirectory = ucutil.AppDataDir("ucwallet", false)
	newlineBytes        = []byte{'\n'}
)
...
// Flags.
var opts = struct {
	TestNet               bool    `long:"testnet" description:"Use the test utopia network"`
	SimNet                bool    `long:"simnet" description:"Use the simulation utopia network"`
...
func init() {
	...
	if opts.TestNet && opts.SimNet {
		fatalf("Multiple utopia networks may not be used simultaneously")
	}
	var activeNet = &chaincfg.MainNetParams
	ucutil.ActiveNet = chaincfg.MainNetParams()
	if opts.TestNet {
...
func makeInputSource(outputs []ucjson.ListUnspentResult) txauthor.InputSource {
	var (
		totalInputValue   ucutil.Amount
		inputs            = make([]*wire.TxIn, 0, len(outputs))
		redeemScriptSizes = make([]int, 0, len(outputs))
		sourceErr         error
	)
	for _, output := range outputs {
		outputAmount, err := ucutil.NewAmount(output.Amount)
		if err != nil {
	...
	return func(ucutil.Amount) (*txauthor.InputDetail, error) {
...
func (src *destinationScriptSourceToAddress) Script() ([]byte, uint16, error) {
	destinationAddress, err := ucutil.DecodeAddress(src.address)
	if err != nil {
		return nil, 0, err
	}
	script, err := txscript.PayToAddrScript(destinationAddress)
...
func sweep() error {
	...
	rpcClient, err := ucrpcclient.New(&ucrpcclient.ConnConfig{
	...
	sourceOutputs := make(map[string][]ucjson.ListUnspentResult)
	...
	feeRate, err := ucutil.NewAmount(opts.FeeRate)
	if err != nil {
	...
	for _, previousOutputs := range sourceOutputs {
		...
		outputAmount := ucutil.Amount(atx.Tx.TxOut[0].Value)
...
func saneOutputValue(amount ucutil.Amount) bool {
	return amount >= 0 && amount <= ucutil.MaxAmount
}

func parseOutPoint(input *ucjson.ListUnspentResult) (wire.OutPoint, error) {
