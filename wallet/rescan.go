...
func NewRescanFilter(addresses []ucutil.Address, unspentOutPoints []*wire.OutPoint) *RescanFilter {
	filter := &RescanFilter{
		pubKeyHashes:        map[[ripemd160.Size]byte]struct{}{},
		scriptHashes:        map[[ripemd160.Size]byte]struct{}{},
		compressedPubKeys:   map[[33]byte]struct{}{},
...
func (f *RescanFilter) AddAddress(a ucutil.Address) {
	switch a := a.(type) {
...
func (f *RescanFilter) ExistsAddress(a ucutil.Address) (ok bool) {
	switch a := a.(type) {
	case *ucutil.AddressPubKeyHash:
...
func (f *RescanFilter) RemoveAddress(a ucutil.Address) {
	switch a := a.(type) {
	case *ucutil.AddressPubKeyHash:
