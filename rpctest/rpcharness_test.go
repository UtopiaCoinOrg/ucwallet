...
func TestMain(m *testing.M) {
	...
	ntfnHandlersNode := ucrpcclient.NotificationHandlers{
...
func testValidateAddress(r *Harness, t *testing.T) {
	...
	for _, acct := range accounts {
		...
		// Decode address
		_, err = ucutil.DecodeAddress(addrStr)
		if err != nil {
			t.Fatalf("Unable to decode address %s: %v", addr.String(), err)
		}

		// Try to validate an address that is not owned by wallet
		otherAddress, err := ucutil.DecodeAddress("SsqvxBX8MZC5iiKCgBscwt69jg4u4hHhDKU")
		if err != nil {
	...
	DevAddr, err := ucutil.DecodeAddress(devSubAddrStr)
	if err != nil {
...
func testWalletPassphrase(r *Harness, t *testing.T) {
	...
	if err != nil && err.(*ucjson.RPCError).Code !=
		ucjson.ErrRPCWalletPassphraseIncorrect {
		// ucjson.ErrWalletPassphraseIncorrect.Code
	...
	// ucjson.ErrRPCWalletUnlockNeeded
	if !strings.HasPrefix(err.Error(),
		strconv.Itoa(int(ucjson.ErrRPCWalletUnlockNeeded))) {
		t.Fatalf("createnewaccount returned error (%v) instead of %v",
			err, ucjson.ErrRPCWalletUnlockNeeded)
	}
	...
	if err != nil && err.(*ucjson.RPCError).Code !=
		ucjson.ErrRPCWalletAlreadyUnlocked {
		t.Fatalf("WalletPassphrase failed: %v", err)
	}
...
func testGetBalance(r *Harness, t *testing.T) {
	...
	sendAmount := ucutil.Amount(700000000)
...
func testListUnspent(r *Harness, t *testing.T) {
	...
	amountToSend := ucutil.Amount(700000000)
	...
	// Get *ucutil.Tx of send to check the inputs
	...
	for _, txIn := range rawTx.MsgTx().TxIn {
		...
		txInIDs[prevOut.String()] = ucutil.Amount(txIn.ValueIn).ToCoin()
	}
	...
	for txinID, amt := range txInIDs {
		if _, ok := utxosBeforeSend[txinID]; !ok {
			t.Fatalf("Failed to find txid %v (%v UC) in list of UTXOs",
				txinID, amt)
		}
	}
...
func testSendFrom(r *Harness, t *testing.T) {
	...
	amountToSend := ucutil.Amount(1000000)
	...
	fee := ucutil.Amount(totalSpent - totalSent)
...
func testSendMany(r *Harness, t *testing.T) {
	...
	amountsToSend := []ucutil.Amount{700000000, 1400000000}
	addresses := []ucutil.Address{}
	...
	addressAmounts := make(map[ucutil.Address]ucutil.Amount)
	totalAmountToSend := ucutil.Amount(0)
...
func testListTransactions(r *Harness, t *testing.T) {
	...
	voutAmt := ucutil.Amount(txOut.Value).ToCoin()
	...
	sendAmount := ucutil.Amount(240000000)
...
func testGetSetRelayFee(r *Harness, t *testing.T) {
	// ucrpcclient does not have a getwalletfee or any direct method, so we
	...
	origTxFee, err := ucutil.NewAmount(walletInfo.TxFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TxFee, err)
	}
	// Increase fee by 50%
	newTxFeeCoin := walletInfo.TxFee * 1.5
	newTxFee, err := ucutil.NewAmount(newTxFeeCoin)
	if err != nil {
	...
	newTxFeeActual, err := ucutil.NewAmount(walletInfo.TxFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TxFee, err)
	}
	...
	err = wcl.SetTxFee(ucutil.Amount(-1))
	if err == nil {
...
func testGetSetTicketFee(r *Harness, t *testing.T) {
	// ucrpcclient does not have a getticketee or any direct method, so we
	...
	origTicketFee, err := ucutil.NewAmount(nominalTicketFee)
	if err != nil {
	...
	newTicketFee, err := ucutil.NewAmount(newTicketFeeCoin)
	if err != nil {
	...
	nominalTicketFee = walletInfo.TicketFee
	newTicketFeeActual, err := ucutil.NewAmount(nominalTicketFee)
	if err != nil {
	...
	priceLimit, err := ucutil.NewAmount(2 * mustGetStakeDiffNext(r, t))
	if err != nil {
	...
	err = wcl.SetTicketFee(ucutil.Amount(-1))
	if err == nil {
...
func testPurchaseTickets(r *Harness, t *testing.T) {
	...
	origTicketFee, err := ucutil.NewAmount(walletInfo.TicketFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TicketFee, err)
	}
	newTicketFee, err := ucutil.NewAmount(walletInfo.TicketFee * 1.5)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TicketFee, err)
	}
...
func mustGetStakeInfo(wcl *ucrpcclient.Client, t *testing.T) *types.GetStakeInfoResult {
	stakeinfo, err := wcl.GetStakeInfo()
	if err != nil {
		t.Fatal("GetStakeInfo failed: ", err)
	}
...
func newBlockAt(currentHeight uint32, r *Harness,
	t *testing.T) (uint32, *ucutil.Block, []*chainhash.Hash) {
...
func newBlockAtQuick(currentHeight uint32, r *Harness,
	...
	return block.Header.Height, ucutil.NewBlock(block), blockHashes
}

func getBestBlock(r *Harness, t *testing.T) (uint32, *ucutil.Block, *chainhash.Hash) {
	...
	return curBlockHeight, ucutil.NewBlock(bestBlock), bestBlockHash
}
...
func includesTx(txHash *chainhash.Hash, block *ucutil.Block) bool {
	if len(block.Transactions()) <= 1 {
		return false
	}
...
// includesTx checks if a block contains a transaction hash
func includesStakeTx(txHash *chainhash.Hash, block *ucutil.Block) bool {
...
// spent minus sent.
func getWireMsgTxFee(tx *ucutil.Tx) ucutil.Amount {
	var totalSpent int64
	...
	return ucutil.Amount(totalSpent - totalSent)
}
