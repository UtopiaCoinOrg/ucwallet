...
import (
	...
	ldr "github.com/UtopiaCoinOrg/ucwallet/loader"
...
func run(ctx context.Context) error {
	...
	// the trusted ucd server.  This blocks until cancelled.
	if !cfg.NoInitialLoad {
		if done(ctx) {
			return ctx.Err()
		}
...
// startChainRPC opens a RPC client connection to a ucd server for blockchain
...
func startChainRPC(ctx context.Context, certs []byte) (*chain.RPCClient, error) {
	...
	rpcc, err := chain.NewRPCClientConfig(activeNet.Params, &rpcclient.ConnConfig{
		Host:                 cfg.RPCConnect,
		Endpoint:             "ws",
		User:                 cfg.UcdUsername,
		Pass:                 cfg.UcdPassword,
		Certificates:         certs,
