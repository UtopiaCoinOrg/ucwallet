...
func normalizeAddress(addr ucutil.Address) ucutil.Address {
...
func (m *Manager) loadAccountInfo(ns walletdb.ReadBucket, account uint32) (*accountInfo, error) {
	...
	acctKeyPub, err := hdkeychain.NewKeyFromString(string(serializedKeyPub), ucutil.ActiveNet)
	if err != nil {
	...
	if !m.locked {
		...
		acctKeyPriv, err := hdkeychain.NewKeyFromString(string(decrypted), ucutil.ActiveNet)
		if err != nil {
			return nil, errors.E(errors.IO, err)
		}
...
func (m *Manager) CoinTypePrivKey(dbtx walletdb.ReadTx) (*hdkeychain.ExtendedKey, error) {
	...
	coinTypeKeyPriv, err := hdkeychain.NewKeyFromString(string(serializedKeyPriv), ucutil.ActiveNet)
...
// now created using the coin types assigned to Utopia in SLIP0044:
//
...
// to the coin type registered to Utopia as per SLIP0044.  On mainnet, this
...
func (m *Manager) UpgradeToSLIP0044CoinType(dbtx walletdb.ReadWriteTx) error {
	...
	acctExtPubKey, err := hdkeychain.NewKeyFromString(string(acctExtPubKeyStr), ucutil.ActiveNet)
	if err != nil {
		return errors.E(errors.IO, err)
	}
	...
	if !m.locked {
		...
		acctExtPrivKey, err = hdkeychain.NewKeyFromString(string(acctExtPrivKeyStr), ucutil.ActiveNet)
		if err != nil {
			return errors.E(errors.IO, err)
		}
	}
...
//
// This function MUST be called with the manager lock held for writes.
func (m *Manager) loadAddress(ns walletdb.ReadBucket, address ucutil.Address) (ManagedAddress, error) {
...
func (m *Manager) Address(ns walletdb.ReadBucket, address ucutil.Address) (ManagedAddress, error) {
...
func (m *Manager) AddrAccount(ns walletdb.ReadBucket, address ucutil.Address) (uint32, error) {
...
func (m *Manager) ImportPrivateKey(ns walletdb.ReadWriteBucket, wif *ucutil.WIF) (ManagedPubKeyAddress, error) {
	defer m.mtx.Unlock()
	m.mtx.Lock()
	...
	pubKeyHash := ucutil.Hash160(serializedPubKey)
...
func (m *Manager) ImportScript(ns walletdb.ReadWriteBucket, script []byte) (ManagedScriptAddress, error) {
	...
	// Prevent duplicates.
	scriptHash := ucutil.Hash160(script)
...
func (m *Manager) MarkUsed(ns walletdb.ReadWriteBucket, address ucutil.Address) error {
...
func (m *Manager) NewAccount(ns walletdb.ReadWriteBucket, name string) (uint32, error) {
	...
	coinTypeKeyPriv, err :=
		hdkeychain.NewKeyFromString(string(serializedKeyPriv), ucutil.ActiveNet)
	zero.Bytes(serializedKeyPriv)
	if err != nil {
...
func (m *Manager) ForEachActiveAddress(ns walletdb.ReadBucket, fn func(addr ucutil.Address) error) error {
	m.mtx.Lock()
	defer m.mtx.Unlock()
...
func (m *Manager) PrivateKey(ns walletdb.ReadBucket, addr ucutil.Address) (key chainec.PrivateKey, done func(), err error) {
...
func (m *Manager) RedeemScript(ns walletdb.ReadBucket, addr ucutil.Address) (script []byte, done func(), err error) {
	// Lock the manager mutex for writes.  This protects read access to m.locked
...
func createWatchOnly(ns walletdb.ReadWriteBucket, hdPubKey string, pubPassphrase []byte, chainParams *chaincfg.Params, config *ScryptOptions) (err error) {
	...
	acctKeyPub, err := hdkeychain.NewKeyFromString(hdPubKey, ucutil.ActiveNet)
	if err != nil {
		// The seed is unusable if the any of the children in the
		// required hierarchy can't be derived due to invalid child.
		if err == hdkeychain.ErrInvalidChild {
