...
import (
	...
	"github.com/UtopiaCoinOrg/ucutil"
...
func setup() error {
	...
	return walletdb.Update(db, func(dbtx walletdb.ReadWriteTx) error {
		...
		wif, err := ucutil.NewWIF(privKey, chainParams, chainec.ECTypeSecp256k1)
		if err != nil {
			return err
		}
...
func createUnsignedTicketPurchase(prevOut *wire.OutPoint,
	inputAmount, ticketPrice ucutil.Amount) (*wire.MsgTx, error) {
	...
	pkScript, err = txscript.GenerateSStxAddrPush(addr,
		ucutil.Amount(amountsCommitted[0]), ticketFeeLimits)
	if err != nil {
		return nil, err
	}
