...
func newWalletConfig(prefix, caFile, certFile, keyFile string, extra []string) (*walletTestConfig, error) {
	...
	a := &walletTestConfig{
		rpcConnect: "127.0.0.1:12509",
		rpcListen:  "127.0.0.1:12510",
		rpcUser:    "user",
		rpcPass:    "pass",
		extra:      extra,
		prefix:     prefix,
...
// command returns the exec.Cmd which will be used to start the ucwallet
// process.
...
// to connect to the ucwallet process that is launched via Start().
...
// process, and as the base for the log and data directories for ucwallet.
...
// Start creates a new ucwallet process, and writes its pid in a file reserved
...
// Stop interrupts the running ucwalletTest process process, and waits until it exits
