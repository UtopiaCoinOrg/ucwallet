...
func (w *Wallet) LiveTicketHashes(chainClient *rpcclient.Client, includeImmature bool) ([]chainhash.Hash, error) {
	...
	// these transactions are not part of the wallet's transaction history, ucd
	// must be queried for their blockchain height.  This functionality requires
	// the ucd transaction index to be enabled.
...
// pruned tickets.
func (w *Wallet) TicketHashesForVotingAddress(votingAddr ucutil.Address) ([]chainhash.Hash, error) {
...
func (w *Wallet) AddTicket(ticket *wire.MsgTx) error {
	...
	err = walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		err := w.StakeMgr.InsertSStx(stakemgrNs, ucutil.NewTx(ticket))
		if err != nil {
			return err
		}
