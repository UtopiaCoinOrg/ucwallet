...
type Store struct {
	...
	acctLookupFunc func(walletdb.ReadBucket, ucutil.Address) (uint32, error)
}
...
// BlockCFilter is a compact filter for a Utopia block.
...
func stakeValidate(ns walletdb.ReadWriteBucket, height int32) error {
	...
	for i := range blockRec.transactions {
		...
		for i, output := range txRec.MsgTx.TxOut {
			...
			minedBalance += ucutil.Amount(output.Value)
		}
...
func (s *Store) AddCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta,
	...
	if invalidated {
		...
		cred := credit{
			...
			block:      block.Block,
			amount:     ucutil.Amount(rec.MsgTx.TxOut[index].Value),
			change:     change,
			spentBy:    indexedIncidence{index: ^uint32(0)},
...
func (s *Store) addCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta,
	...
	if block == nil {
		...
		v := valueUnminedCredit(ucutil.Amount(rec.MsgTx.TxOut[index].Value),
	...
	txOutAmt := ucutil.Amount(rec.MsgTx.TxOut[index].Value)
...
func (s *Store) AddMultisigOut(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta, index uint32) error {
	...
	val = valueMultisigOut(p2shScriptHash,
		...
		block.Block.Hash,
		uint32(block.Block.Height),
		ucutil.Amount(rec.MsgTx.TxOut[index].Value),
		*empty,     // Unspent
		0xFFFFFFFF, // Unspent
		rec.Hash)
...
func (s *Store) UnspentMultisigCreditsForAddress(ns walletdb.ReadBucket, addr ucutil.Address) ([]*MultisigCredit, error) {
...
// without error.
func (s *Store) UnspentOutputsForAmount(ns, addrmgrNs walletdb.ReadBucket, needed ucutil.Amount, syncHeight int32, minConf int32, all bool, account uint32) ([]*Credit, error) {
	var eligible []*minimalCredit
	var toUse []*minimalCredit
	var unspent []*Credit
	found := ucutil.Amount(0)
...
type InputSource struct {
	source func(ucutil.Amount) (*txauthor.InputDetail, error)
}
...
func (s *InputSource) SelectInputs(target ucutil.Amount) (*txauthor.InputDetail, error) {
...
func (s *Store) MakeIgnoredInputSource(ns, addrmgrNs walletdb.ReadBucket, account uint32, minConf,
	...
	var (
		currentTotal      ucutil.Amount
		currentInputs     []*wire.TxIn
		currentScripts    [][]byte
		redeemScriptSizes []int
	)

	f := func(target ucutil.Amount) (*txauthor.InputDetail, error) {
...
type Balances struct {
	Account                 uint32
	ImmatureCoinbaseRewards ucutil.Amount
	ImmatureStakeGeneration ucutil.Amount
	LockedByTickets         ucutil.Amount
	Spendable               ucutil.Amount
	Total                   ucutil.Amount
	VotingAuthority         ucutil.Amount
	Unconfirmed             ucutil.Amount
}
...
func (s *Store) TotalInput(dbtx walletdb.ReadTx, tx *wire.MsgTx) (ucutil.Amount, error) {
	ns := dbtx.ReadBucket(wtxmgrBucketKey)

	var total ucutil.Amount
	for _, in := range tx.TxIn {
		...
		total += ucutil.Amount(tx.TxOut[in.PreviousOutPoint.Index].Value)
	}
