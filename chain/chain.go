...
// RPCClient represents a persistent client connection to a utopia RPC server
...
// operate on the same utopia network as described by the passed chain
...
func (c *RPCClient) Start(ctx context.Context, retry bool) (err error) {
	...
	if err != nil {
		return errors.E(op, errors.E(errors.Op("ucd.jsonrpc.getcurrentnet"), err))
	}
	...
	if err == nil {
		versionResult := versions["ucdjsonrpcapi"]
		serverAPI = semver{
...
// utopia RPC server.  This channel must be continually read or the process
...
out:
	for {
		...
		case <-pingChan:
			...
			// be due to ucd processing websocket requests synchronously (see
