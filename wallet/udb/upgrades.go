...
const (
	...
	// assigned to Utopia in SLIP0044.  The upgrade does not add or remove any
	...
	// store compact filters, which are required for Utopia's SPV
...
func lastUsedAddressIndexUpgrade(tx walletdb.ReadWriteTx, publicPassphrase []byte, params *chaincfg.Params) error {
	...
	for account := uint32(0); account <= lastAccount; account++ {
		...
		xpub, err := hdkeychain.NewKeyFromString(string(serializedKeyPub), ucutil.ActiveNet)
		if err != nil {
		...
		for child := uint32(0); child < hdkeychain.HardenedKeyStart; child++ {
			...
			// ucutil.NewAddressPubKeyHash.  Also, while it looks like a
