...
// Harness fully encapsulates an active ucd process, along with an embedded
// ucwallet to provide a unified platform for creating RPC-driven integration
// tests involving ucd. The active ucd node will typically be run in simnet
...
// spends. The active ucd process is fully managed by Harness, which handles
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	...
	// Start ucwallet. This spawns a new process which will be managed
	...
	// Get a new address from the wallet to be set with ucd's --miningaddr
	time.Sleep(5 * time.Second)
	var miningAddr ucutil.Address
	for i := 0; i < 100; i++ {
...
// ucd process belonging to this Harness instance. If the initial connection
