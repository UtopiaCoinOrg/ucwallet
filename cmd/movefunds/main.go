...
type configJSON struct {
	...
	UcctlArgs    string `json:"ucctlargs"`
}
...
func makeInputSource(outputs []ucjson.ListUnspentResult) (ucutil.Amount, txauthor.InputSource) {
	var (
	...
	return totalInputValue, func(ucutil.Amount) (*txauthor.InputDetail, error) {
...
func main() {
	...
	var utxos []ucjson.ListUnspentResult
	...
	addr, err := ucutil.DecodeAddress(cfg.SendToAddress)
	if err != nil {
		fmt.Printf("failed to parse address %s: %v", cfg.SendToAddress, err)
		return
	}

	fee, err := ucutil.NewAmount(float64(cfg.TxFee))
	if err != nil {
	...
	buf.WriteString("ucctl ")
	buf.WriteString(cfg.UcctlArgs)
