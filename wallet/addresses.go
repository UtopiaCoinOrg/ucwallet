...
func (x *xpubAddress) rewardCommitment(amount ucutil.Amount, limits uint16) ([]byte, uint16) {
	s := make([]byte, 32)
...
func voteRightsScript(addr ucutil.Address) (script []byte, version uint16, err error) {
	switch addr := addr.(type) {
	case stakeAddress:
...
func ticketChangeScript(addr ucutil.Address) (script []byte, version uint16, err error) {
	switch addr := addr.(type) {
	case stakeAddress:
...
func rewardCommitment(addr ucutil.Address, amount ucutil.Amount, limits uint16) (script []byte, version uint16, err error) {
	switch addr := addr.(type) {
	case stakeAddress:
...
func payVoteCommitment(addr ucutil.Address) (script []byte, version uint16, err error) {
	switch addr := addr.(type) {
	case stakeAddress:
...
func payRevokeCommitment(addr ucutil.Address) (script []byte, version uint16, err error) {
	switch addr := addr.(type) {
	case stakeAddress:
...
func (w *Wallet) nextAddress(op errors.Op, persist persistReturnedChildFunc, account, branch uint32,
	callOpts ...NextAddressCallOption) (ucutil.Address, error) {
...
func (w *Wallet) watchFutureAddresses(dbtx walletdb.ReadTx) error {
	...
	for account, a := range w.addressBuffers {
		...
		addrs := make([]ucutil.Address, 0, totalAddrs)
...
func (w *Wallet) NewExternalAddress(account uint32, callOpts ...NextAddressCallOption) (ucutil.Address, error) {
...
func (w *Wallet) NewInternalAddress(account uint32, callOpts ...NextAddressCallOption) (ucutil.Address, error) {
	const op errors.Op = "wallet.NewExternalAddress"
	return w.nextAddress(op, w.persistReturnedChild(nil), account, udb.InternalBranch, callOpts...)
}

func (w *Wallet) newChangeAddress(op errors.Op, persist persistReturnedChildFunc, account uint32) (ucutil.Address, error) {
...
// policy.
func (w *Wallet) NewChangeAddress(account uint32) (ucutil.Address, error) {
...
func (w *Wallet) AccountBranchAddressRange(account, branch, start, end uint32) ([]ucutil.Address, error) {
...
func deriveChildAddresses(key *hdkeychain.ExtendedKey, startIndex, count uint32, params *chaincfg.Params) ([]ucutil.Address, error) {
	addresses := make([]ucutil.Address, 0, count)
...
func deriveChildAddress(key *hdkeychain.ExtendedKey, child uint32, params *chaincfg.Params) (ucutil.Address, error) {
...
func appendChildAddrsRange(addrs *[]ucutil.Address, key *hdkeychain.ExtendedKey,
